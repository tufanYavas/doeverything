...
export const createStorage = <D = string>(
  ...
  // Wrap so callers can `await` regardless of whether the user-provided
  // serializer is sync or async — the rest of the code paths await results.
  const serializeAsync = async (v: D): Promise<unknown> => serialize(v);
  const deserializeAsync = async (v: unknown): Promise<D> => deserialize(v);
  ...
  const get = async (): Promise<D> => {
    ...
    return (await deserializeAsync(value[key])) ?? fallback;
  };
  ...
  const set = async (valueOrUpdate: ValueOrUpdateType<D>) => {
    ...
    await chrome?.storage[storageEnum].set({ [key]: await serializeAsync(cache) });
    _emitChange();
  };
  ...
  const _updateFromStorageOnChanged = async (changes: { [key: string]: chrome.storage.StorageChange }) => {
    ...
    const valueOrUpdate: ValueOrUpdateType<D> = await deserializeAsync(changes[key].newValue);
